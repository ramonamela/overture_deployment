...
def deploy_minio(base_minio_vm, minio_port, minio_username, minio_password, minio_base_image, container_name=None,
    ...
    base_minio_vm.run_command(deploy_command, print_message="Deploy the minio service", verbose=verbose)
...
def initialize_s3_bucket_minio(vm, minio_user, minio_pass, minio_ip, minio_port, s3_name, s3_region, host_working_path,
    ...
    init_command = "docker run --env AWS_ACCESS_KEY_ID=" + minio_user + " --env AWS_SECRET_ACCESS_KEY=" + minio_pass \
                   ...
                   + "http://" + minio_ip + ":" + minio_port + " s3 mb s3://" + s3_name

    if verbose:
        print(init_command)
    vm.run_command(init_command, print_message="Initialize s3 bucket minio", verbose=verbose)
...
def deploy_score_server(vm, score_username, score_password, score_port, song_address, song_port, s3_name, s3_endpoint,
                        s3_port, s3_username, s3_password, auth_url, score_base_image, container_name=None,
                        enable_ssl="false", upload_part_size=1073741824, upload_con_timeout=1200000, log_level="debug",
                        verbose=False):
    ...
    if verbose:
        print(deploy_command)
    vm.run_command(deploy_command, print_message="Start postgres docker", verbose=verbose)


def deploy_song_server(vm, song_username, song_password, song_port, host_working_path, postgres_username,
                       postgres_password, postgres_url, score_url, score_token, auth_url, song_base_image,
                       container_name=None, verbose=False):
    python_command = "from pathlib import Path;" \
                     "from os import symlink;" \
                     "db_working_path = \"" + host_working_path + "\";" \
    ...
    deploy_command = "docker run -d -v " + host_working_path + ":/song-server/logs " \
                     + " --env SERVER_PORT=8080 " \
                     + " --env SPRING_PROFILES_ACTIVE=\"prod,secure,default\" " \
                     + " --env AUTH_SERVER_URL=" + auth_url \
                     + " --env AUTH_SERVER_CLIENTID=" + song_username \
                     + " --env AUTH_SERVER_CLIENTSECRET=" + song_password \
                     + " --env AUTH_SERVER_SCOPE_STUDY_PREFIX=song." \
                     + " --env AUTH_SERVER_SCOPE_STUDY_SUFFIX=.WRITE" \
                     + " --env AUTH_SERVER_SCOPE_SYSTEM=song.WRITE" \
                     + " --env SCORE_URL=" + score_url \
                     + " --env SCORE_ACCESSTOKEN=" + score_token \
                     + " --env MANAGEMENT_SERVER_PORT=8081" \
                     + " --env SPRING_DATASOURCE_USERNAME=" + postgres_username \
                     ...
                     + " --env ID_USELOCAL=\"true\""

    deploy_command += " -p " + song_port + ":8080 "
    ...
    if container_name is not None:
        ...
        try:
            ...
            if "No such container" not in str(e):
                raise e
        deploy_command += " --name " + container_name
    deploy_command += " " + song_base_image

    vm.run_command(deploy_command, print_message="Create all necessary folders", verbose=verbose)
